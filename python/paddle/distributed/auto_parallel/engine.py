...
class Engine:
    ...
    def __init__(
        ...
        if (model and not isinstance(model, paddle.nn.Layer)
        ...
        if (loss and not isinstance(loss, (paddle.nn.Layer, Variable))
        ...
        for metric in to_list(metrics):
            assert isinstance(metric,
                              Metric), "{} is not sub class of Metric".format(
    ...
    def _prepare_data_tensor(self, inputs_spec, labels_spec, inputs, labels):
        ...
        if inputs_spec:
            assert isinstance(
                inputs_spec,
                list), "inputs should be list, but received {}".format(
                    type(inputs_spec))
            assert isinstance(
                inputs, list), "inputs should be list, but received {}".format(
                    type(inputs))
        ...
        if labels_spec:
            assert isinstance(
                labels_spec,
                list), "labels should be list, but received {}".format(
                    type(labels_spec))
            assert isinstance(
                labels, list), "labels should be list, but received {}".format(
                    type(labels))
    ...
    def _prepare_feed(self, data, user_feeds, mode):
        ...
        if user_feeds is not None:
            assert isinstance(
                user_feeds,
                dict), "user_feeds must be a dict, but receive {}".format(
    ...
    def _prepare_fetch(self, user_fetches, mode):
        if user_fetches is not None:
            assert isinstance(
                user_fetches,
                list), "user_fetches must be a list, but receive {}".format(
    ...
    def _build(self, mode):
        ...
        if _non_static_mode() or self._dygraph_mode:
            ...
            if not self._skip_build:
                with static.program_guard(
                        serial_main_prog,
                        serial_startup_prog), utils.unique_name.guard():
                    ...
                    # print("****serial_main_prog****", serial_main_prog)
        ...
        self._fw_dist_contexts[mode] = DistributedContext(
            serial_main_prog,
            serial_startup_prog,
            self._optimizer,
            self._losses,
    ...
    def prepare(
        ...
        if inputs or labels:
            ...
            inputs, labels = self._prepare_data_tensor(inputs_spec, labels_spec,
                                                       inputs, labels)
