...
class AMPState(object):
    ...
    def _insert_cast_ops(self, dist_context):
        ops = self._block.ops
        idx = 0
        while idx < len(ops):
            op = ops[idx]
            ...
            if self._is_fp16_op(op.desc.id()) == False:
                num_cast_ops = self._insert_cast_op(
    ...
    def _insert_cast_op(self, op, idx, src_dtype, dst_dtype, dist_context):
        """
    ...
    def _insert_cast_op_backward(self, grad_op, idx, src_dtype, dst_dtype,
        ...
        def _keep_fp32_output(op, out_name):
            op_type = op.type
            if op_type in ['layer_norm']:
                return out_name != 'X@GRAD'
            return False

        num_cast_ops = 0
        dist_op_context = dist_context.dist_op_context
        if grad_op.desc.id() not in dist_op_context.grad_op_id_to_op_id:
            return num_cast_ops
        ...
        for out_name in grad_op.output_names:
            ...
            for out_var_name in grad_op.output(out_name):
                ...
                # NOTE: the out_var's dtype of consume grad_op should equal to the fwd_var
                ...
                if out_var.dtype == src_dtype:
                    if out_var_name_prefix in self._var_name_dict[fwd_op_id]:
                        ...
                        # convert the cast_var to the original out_var
                        consume_op_attr = dist_context.get_op_dist_attr_for_program(
                            grad_op)
...
class AMPPass(PassBase):
    ...
    def _apply_single_impl(self, main_program, startup_program, context):
        ...
        amp_state._insert_cast_ops(self._dist_context)
    ...
    def _scale_loss(self):
        ...
        loss_op = loss.op
        loss_op_dist_attr = self._dist_context.get_op_dist_attr_for_program(
            loss_op)
